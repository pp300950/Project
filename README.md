...
| `LINE_TOKEN` | `Bearer xxxx...` | Channel Access Token จาก LINE Developers |
| `LINE_USER_ID` | `Uxxxxxxxx` | User ID ที่จะรับการแจ้งเตือน |
...
- **3** — ยิงเหตุการณ์วิกฤต เช็คว่า LINE ส่งได้ไหม
...
## 4. LINE Messaging API (วิธีดึง Token)

1. ไปที่ https://developers.line.biz → **Create a Provider**
2. สร้าง **Messaging API channel**
3. ใน channel settings → **Messaging API** tab
4. Copy **Channel access token (long-lived)** → ใส่เป็น `LINE_TOKEN`
5. ดึง **User ID** ของตัวเอง:
   - เปิด LINE app → เพิ่มเพื่อน bot ที่สร้าง
   - ส่งข้อความหาบอท 1 ครั้ง
   - ใน Render Logs จะเห็น userId ใน webhook request
   - หรือใช้ LINE Official Account Manager → ดู User ID

---

## 5. สรุปโครงสร้างข้อมูลที่ส่งจาก ESP8266
...
## 6. Local Development (ทดสอบบนเครื่องตัวเอง)
