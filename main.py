...
ENV ที่ต้องตั้งใน Render Dashboard:
  ...
  LINE_TOKEN        = (Bearer token จาก LINE Developers)
...
def init_db():
    ...
    try:
        with conn.cursor() as cur:
            ...
            # ตาราง alerts — เก็บประวัติการส่ง LINE
            cur.execute("""
...
async def send_line_alert(message: str, event_id: int):
    """ส่งข้อความแจ้งเตือนผ่าน LINE Messaging API"""
    if not LINE_TOKEN:
        return

    # ดึง user_id จาก env หรือ hardcode ชั่วคราว
    ...
    try:
        ...
        # บันทึกลง alerts table
        conn = get_conn()
        try:
            with conn.cursor() as cur:
                cur.execute(
                    "INSERT INTO alerts(event_id, channel, status) VALUES(%s, 'LINE', %s)",
                    (event_id, status_text)
                )
            conn.commit()
        except Exception:
            pass
...
async def receive_event(body: SensorEvent):
    """
    ...
    # ส่ง LINE alert สำหรับ event ที่ต้องแจ้งเตือน
    if body.event_type == "zone_breach":
        zone_label = "รั้วบ้าน🏚" if body.zone == "fence" else "สระว่ายน้ำ🏊"
        dist_txt   = f"\n📏 ระยะ {body.distance_cm:.1f} cm" if body.distance_cm else ""
        msg = (
            f"❗❗ แจ้งเตือน ❗❗\n"
            f"📌 ตรวจพบผู้ป่วย ณ\n"
            f"บริเวณ {zone_label}{dist_txt}\n"
            f"🕐 เวลา: {ts_str}\n"
            f"📟 อุปกรณ์: {body.device_id}"
        )
        await send_line_alert(msg, event_id)

    elif body.event_type == "shoe_removed":
        side = ""
        if body.shoe_left is False and body.shoe_right is False:
            side = "ทั้งสองข้าง"
        elif body.shoe_left is False:
            side = "ข้างซ้าย"
        elif body.shoe_right is False:
            side = "ข้างขวา"
        msg = (
            f"⚠️ แจ้งเตือนถอดรองเท้า\n"
            f"👟 ถอดรองเท้า {side}\n"
            f"🕐 เวลา: {ts_str}\n"
            f"📟 อุปกรณ์: {body.device_id}"
        )
