...
/* ── API Base URL ──────────────────────────────────────────
   อ่านจาก localStorage ก่อน (ผู้ดูแลระบบตั้งผ่าน login หน้า)
   ถ้าไม่มีใช้ origin ปัจจุบัน (เมื่อ deploy บน Render)
   ─────────────────────────────────────────────────────────── */
const API_BASE = (() => {
  const stored = localStorage.getItem('ssm_api_base');
  if (stored) return stored.replace(/\/$/, '');
  // ถ้ารันบน file:// (dev โดยไม่มี server) ให้ fallback ไป localhost
